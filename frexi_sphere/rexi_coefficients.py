...
#
# See e.g. Near optimal rational approximations of large data sets,
# Damle et. al.
#
...
def b_coefficients(h, M, n=0):
    if n == 0:
        return [math.exp(h*h)*cmath.exp(-1j*(float(m)*h))
        ...
        b = []
        for m in range(-M, M+1):
            re, err = integrate.quadrature(
                expr_real, max(-1./(2*pi), -1./(2*h)), 0,
                tol=1.e-10, miniter=300)
            im, err = integrate.quadrature(
                expr_imag, max(-1./(2*pi), -1./(2*h)), 0,
                tol=1.e-10, miniter=300)
